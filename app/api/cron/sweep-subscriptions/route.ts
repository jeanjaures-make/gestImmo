 ...
 *
 * Déclenché par Vercel Cron (voir `vercel.json`). Trois corrections :
 *
 ...
 *   — les inscriptions jamais payées, ou payées puis jamais
 *     provisionnées, sont abandonnées de la même façon.
 *
...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await admin
    ...
    .maybeSingle<{ expired: number; abandoned: number; expired_intents: number }>();
  ...
  return NextResponse.json({
    ...
    expired_intents: data?.expired_intents ?? 0,
  });
}
